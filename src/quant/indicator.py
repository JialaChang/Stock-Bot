...
def compute_indicators(ticker: str, name: str, history_data: pd.DataFrame, intraday_data: pd.DataFrame, latest_time) -> StockSnapshot:
    ...
    history_data['EMA_5'] = pta.ema(prices, length=5) # pyright: ignore[reportPrivateImportUsage]
    history_data['EMA_10'] = pta.ema(prices, length=10) # pyright: ignore[reportPrivateImportUsage]
    history_data['EMA_20'] = pta.ema(prices, length=20) # pyright: ignore[reportPrivateImportUsage]
